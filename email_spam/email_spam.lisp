...
(defvar *feature-databases* (make-hash-table :test #'equal))

(defun clear-database ()
  (setf *feature-databases* (make-hash-table :test #'equal)
	*total-spams* 0
	*total-hams* 0))

(defun intern-feature (word)
  (or (gethash word *feature-databases*)
      (setf (gethash word *feature-databases*)
...
(defun false-neganive-p (result)
  (eql (result-type result) 'false-negagive))
...
(defun show-summary (file text classification score)
  (format t "~&~a" file)
  (format t "~2%~a~2%" text)
  (format t "Classified as ~a with scroe of ~,5f~%" classification score))
